...
import sys
sys.path.append('../Grasping_state_assessment_master/utils/progress/progress/')
import bar
from bar import Bar
sys.path.append('../Grasping_state_assessment_master/utils/')
import logger,misc
...
def main():
    ...
    transform_f = transforms.Compose([transforms.Resize([4, 4]),
                                ...
                                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    log=6
    trainset = MyDataset('../Grasping_state_assessment_master/graspingdata',6,3,3,transform_v,transform_t,transform_f,log,flag = opt.phase)
    ...
    opt.phase = 'test'
    validset = MyDataset('../Grasping_state_assessment_master/graspingdata',6,3,3,transform_v,transform_t,transform_f,log,flag = opt.phase)
    ...
    # Model
    if opt.model_arch == 'C3D':
        model = C3D(drop_p_v=0.2, visual_dim=2048, fc_hidden_1=128,fc_hidden_t = 20,fc_hidden_f = 20,num_classes=2)
...
def train(trainloader, model, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (x_visual,x_tactile,x_flex, targets) in enumerate(trainloader):
        # measure data loading time
        data_time.update(time.time() - end)

        x_tactile,x_visual,x_flex, targets = torch.autograd.Variable(x_tactile),torch.autograd.Variable(x_visual),torch.autograd.Variable(x_flex), torch.autograd.Variable(targets)
        if use_cuda:
        ...
        # compute output
        outputs = model(x_visual,x_tactile,x_flex)
...
def valid(testloader, model, epoch, use_cuda):
    ...
    for batch_idx, (x_visual, x_tactile,x_flex, targets) in enumerate(testloader):
        # measure data loading time
        data_time.update(time.time() - end)

        x_tactile, x_visual,x_flex, targets = torch.autograd.Variable(x_tactile), torch.autograd.Variable(
            x_visual),torch.autograd.Variable(x_flex), torch.autograd.Variable(targets)
        if use_cuda:
            x_tactile = x_tactile.cuda()
            x_visual = x_visual.cuda()
            x_flex = x_flex.cuda()
    ...
    fileName='E:/github/fu_Experimental_results/'+ 'C3D' + '-' +'y_pred.txt'
    fw = open(fileName, 'w') 
    fw.write(Ypred)               
    fw.write("\n") 
    fw.close
    
    filename='E:/github/fu_Experimental_results/'+ 'C3D' + '-' +'y_targets.txt'
    FW = open(filename, 'w') 
    FW.write(Ytargets)
    FW.write("\n") 
    FW.close
