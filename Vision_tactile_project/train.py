...
import sys
sys.path.append('../fu_vision_tactile_project/utils/progress/progress/')
import bar
from bar import Bar
sys.path.append('../fu_vision_tactile_project/utils/')
import logger,misc
...
def main():
    ...
    transform_t = transforms.Compose([transforms.Resize([4, 4]),
                                ...
                                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    log=6
    trainset = MyDataset('../fu_vision_tactile_project/graspingdata',6,3,transform_v,transform_t,log,flag = opt.phase)
    ...
    opt.phase = 'test'
    validset = MyDataset('../fu_vision_tactile_project/graspingdata',6,3,transform_v,transform_t,log,flag = opt.phase)
    ...
    # Model
    if opt.model_arch == 'C3D_vision_C1D_tactile':
        model = C3D_vision_C1D_tactile(drop_p_v=0.2,visual_dim=1024,  fc_hidden_1=128,fc_hidden_t = 20,num_classes=2) #visual_dim=4096
...
def train(trainloader, model, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (x_visual,x_tactile, targets) in enumerate(trainloader):
        ...
        x_visual,x_tactile, targets = torch.autograd.Variable(x_visual), torch.autograd.Variable(x_tactile),torch.autograd.Variable(targets)
        if use_cuda:
            # inputs = inputs.cuda()
       ...
       # sklearn.metrics.recall_score(y_true, y_pred, *, labels=None, pos_label=1, average='binary', sample_weight=None, zero_division='warn')召回率
...
def valid(testloader, model, epoch, use_cuda):
    ...
    for batch_idx, ( x_visual,x_tactile, targets) in enumerate(testloader):
        ...
        bar.next()
    bar.finish()
    fileName='E:/github/fu_Experimental_results/'+ 'C3D_vision_C1D_tactile' + '-' +'y_pred.txt'
    fw = open(fileName, 'w') 
    fw.write(Ypred)               
    fw.write("\n") 
    fw.close
    
    filename='E:/github/fu_Experimental_results/'+ 'C3D_vision_C1D_tactile' + '-' +'y_targets.txt'
    FW = open(filename, 'w') 
    FW.write(Ytargets)
    FW.write("\n") 
    FW.close
