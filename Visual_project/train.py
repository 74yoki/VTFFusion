...
from xela_dataloaderdemo01 import MyDataset
import sys
sys.path.append('../fu_visual_project/utils/progress/progress/')
import bar
from bar import Bar
sys.path.append('../fu_visual_project/utils/')
import logger,misc
...
def main():
    ...
    log=1
    trainset = MyDataset('../fu_visual_project/graspingdata',6,transform_v,log,flag = opt.phase)     
    ...
    opt.phase = 'test'
    validset = MyDataset('../fu_visual_project/graspingdata',6,transform_v,log,flag = opt.phase)
...
def train(trainloader, model, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (x_visual, targets) in enumerate(trainloader):
        # measure data loading time
        data_time.update(time.time() - end)

        x_visual, targets = torch.autograd.Variable(x_visual), torch.autograd.Variable(targets)
        if use_cuda:
...
def valid(testloader, model, epoch, use_cuda):
    ...
    for batch_idx, (x_visual,  targets) in enumerate(testloader):
        # measure data loading time
        data_time.update(time.time() - end)

        x_visual, targets =  torch.autograd.Variable(
    ...
    fileName='E:/github/fu_Experimental_results/'+ 'C3D_visual_only' + '-' +'y_pred.txt'
    fw = open(fileName, 'w') 
    fw.write(Ypred)               
    fw.write("\n") 
    fw.close
    
    filename='E:/github/fu_Experimental_results/'+ 'C3D_visual_only' + '-' +'y_targets.txt'
    FW = open(filename, 'w') 
    FW.write(Ytargets)
    FW.write("\n") 
    FW.close
